...
AFRAME.registerComponent('textarea', {
  schema: {
    ...
    selectionColor: { type: 'color', default: 'purple' },
  ...
  init: function () {
    ...
    console.warn("I am in init textarea");
    this.text = null;
    this.lines = [];
    this.lastBlink = 0;
    ...
    // Add an event listener for the 'loaded' event on the background element
    this.background.addEventListener("model-loaded", () => {
    ...
    this.textAnchor.setAttribute("text", {
      value: "testtt ing text here",
      align: "center",
      baseline: "top",
      wrapCount: this.data.cols,
      width: this.data.cols,
    ...
    // this.textAnchor = createGeometry({
    //     mode: 'pre',
    //     baseline: 'top',
    //     anchor: 'center',
    //     font: 'dejavu',
  ...
  _updateCursorGeometry: function () {
    ...
    this.cursorMesh.geometry = mergeBufferGeometries(geos);
